...
#define DEFAULT_MINIMUMFEEDRATE       0.0 //0.0     // (mm/s. °/s for rotational-only moves) Minimum feedrate. Set with M205 S.
#define DEFAULT_MINTRAVELFEEDRATE     0.0 //0.0     // (mm/s. °/s for rotational-only moves) Minimum travel feedrate. Set with M205 T.
...
//Microstep 설정 (X_MS1, X_MS2 등이라는 핀이있는 보드가 필요합니다).
...
/**
* 최소 스테퍼 드라이버 펄스 폭 (µs)
* 0 : MCU가 생산할 수있는 가장 작은 폭, TMC2XXX 드라이버와 호환됩니다.
* 0 : LV8729의 최소 500NS, Stepper.h에서 조정
* 1 : A4988 및 A5984 스테퍼 드라이버의 최소값
* 2 : DRV8825 스테퍼 드라이버의 경우 최소
* 3 : TB6600 스테퍼 드라이버의 경우 최소
* 30 : TB6560 스테퍼 드라이버의 경우 최소
*
* configuration에서 설정된 드라이버 유형을 기준으로 기본값을 대체하십시오. h.
 */
#define MINIMUM_STEPPER_PULSE 1 // 2

/**
 *스테퍼 드라이버가 허용하는 최대 스테핑 속도 (HZ)
* 정의되지 않은 경우 기본값은 1MHz / (2 * MINUMUM_STEPPER_PULSE)
* 5000000 : TMC2XXX 스테퍼 드라이버의 경우 최대
* 10000000 : LV8729 스테퍼 드라이버의 경우 최대
* 500000 : A4988 스테퍼 드라이버의 경우 최대
* 250000 : DRV8825 스테퍼 드라이버의 경우 최대
* 150000 : TB6600 스테퍼 드라이버의 경우 최대
* 15000 : TB6560 스테퍼 드라이버의 경우 최대
 *
 * Override the default value based on the driver type set in Configuration.h.
 */
