...
/*   LOOP_NUM_AXES(i)  {
    if ( (seen[i] = parser.seenval(AXIS_CHAR(i))) ) {
      ...
      if( v == 0 && ( parser.seenval(AXIS_CHAR(X_AXIS)) &&  parser.seenval(AXIS_CHAR(Y_AXIS))) ){
        destination[X_AXIS] = 270;
  ...
  LOOP_NUM_AXES(i) {
    if ( (seen[i] = parser.seenval(AXIS_CHAR(i))) ) {
      ...
      if (skip_move) {
        ...
        if (i == X_AXIS && v == 0 && parser.seenval(AXIS_CHAR(Y_AXIS)) && parser.value_axis_units((AxisEnum)Y_AXIS) == 0) {
          destination[X_AXIS] = R_LINE_OFFSET;
          destination[Y_AXIS] = 0;
        } else {
          destination[i] = axis_is_relative(AxisEnum(i)) ? current_position[i] + v : LOGICAL_TO_NATIVE(v, i);
        }
