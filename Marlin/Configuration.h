...
#define STRING_CONFIG_H_AUTHOR "THEHEIM" // Who made the changes.
...
#define CUSTOM_MACHINE_NAME "HOUSING"

// Printer's unique ID, used by some programs to differentiate between machines.
// Choose your own or use a service like https://www.uuidgenerator.net/version4
#define MACHINE_UUID "test01"
 ...
 *
* 회전하거나 피벗하는 모든 축에 대해 Axisn_Rotates를 정의하십시오.
* 회전 축 좌표는 각도로 표현됩니다.
 *
...
/**
* 움직이는 부분에 연결되는 압출기가있는 두 개의 별도 X- 계산
* 움직임을 사용하여 자기 도킹 메커니즘을 통해, 솔레노이드가 없음
 *
...
//#define MAGNETIC_PARKING_EXTRUDER
  ...
  #define PARKING_EXTRUDER_GRAB_DISTANCE 1            // (mm) Distance to move beyond the parking point to grab the extruder
    ...
    #define MPE_FAST_SPEED      9000      // (mm/min) 마지막 거리 지점 이전의 여행 속도
    #define MPE_SLOW_SPEED      4500      // (mm/min) 마지막 거리를위한 속도 공원 및 커플로 이동
    #define MPE_TRAVEL_DISTANCE   10      // (mm) 마지막 거리 지점
    #define MPE_COMPENSATION       0      //오프셋 보상 -1, 0, 1 (승수) 만 커플 링을 위해서만
...
#define MAX_CHAMBER_POWER 255 // limits duty cycle to chamber heater; 255=full current
  ...
  #define PID_FUNCTIONAL_RANGE 10 // 목표 온도와 실제 온도의 온도 차이가
...
//#define DELTA
  ...
  #define PRINTABLE_RADIUS       60000.0    // (mm)

  // Maximum reachable area
  #define DELTA_MAX_RADIUS       120000.0    // (mm)
...
#define POLAR
 ...
 /*  USE : DEFAULT_SEGMENTS_PER_SECOND, PRINTABLE_RADIUS, R_LINE_OFFSET, */

#if ENABLED(POLAR)
  #define DEFAULT_SEGMENTS_PER_SECOND 180 // 180   // If movement is choppy try lowering this value
  #define PRINTABLE_RADIUS 1900.0f // 82.0f            // (mm) Maximum travel of X axis
 ...
 */
// #define DEFAULT_AXIS_STEPS_PER_UNIT   { 330.75, 1109.57, 200, 415 }
#define DEFAULT_AXIS_STEPS_PER_UNIT {330, 800, 200, 415}
...
/**
 * 기본 맥스 가속 (시간에 따른 속도 변화) (선형 = mm/(s^2), 회전 = °/(s^2))
* (가속 이동의 최대 시작 속도)
 * Override with M201
 *                                      X, Y, Z [, I [, J [, K...]]], E0 [, E1[, E2...]]
 */
#define DEFAULT_MAX_ACCELERATION      { 250, 250, 100, 10000 }
...
/**
 *기본 가속도 (시간에 따른 속도 변화) (선형 = mm/(s^2), 회전 = °/(s^2))
* M204로 무시하십시오
 *
 *   M204 P    Acceleration
 ...
 */
#define DEFAULT_ACCELERATION          200    // 인쇄를위한 X, Y, Z 및 E 가속도
#define DEFAULT_RETRACT_ACCELERATION  3000    //후퇴에 대한 가속도
#define DEFAULT_TRAVEL_ACCELERATION   100    //x, y, z 여행에 대한 가속도 (비 인쇄) 이동

/**
* 기본 저크 제한 (mm/s)
* M205 x y z로 무시하십시오...이자형
*
* "바보"는 가속이 필요한 최소 속도 변화를 지정합니다.
* 속도와 방향을 변경할 때 차이가
* 여기에 설정하면 즉시 발생할 수 있습니다.
*/
#define CLASSIC_JERK
#if ENABLED(CLASSIC_JERK)
  #define DEFAULT_XJERK 0.2  //10.0
  #define DEFAULT_YJERK 0.2  //10.0
  ...
  //#define TRAVEL_EXTRA_XYJERK 0.0     // 모든 여행 이동에 대한 추가 저크 허용

  //#define LIMITED_JERK_EDITING        // M205 또는 LCD를 통해 편집을 제한하여 default_ajerk *2
...
/*** S- 커브 가속도
*
*이 옵션은 Bézier를 장착하여 인쇄하는 동안 진동을 제거합니다.
* 가속도를 움직이기 위해 곡선으로, 훨씬 더 부드러운 방향 변화가 생성됩니다.
*
* https://github.com/synthetos/tinyg/wiki/jerk-controlled-motion-emplained를 참조하십시오
*/
...
/**
* Z-Min 핀에 연결된 프로브의 경우이 옵션을 활성화하십시오.
* 프로브는 Z-Min 엔드 스톱을 대체하며 Z Homing에 사용됩니다.
* (ASTOMIALS aute_probe_for_z_homing을 활성화합니다.)
*/
...
#define X_BED_SIZE PRINTABLE_RADIUS
#define Y_BED_SIZE PRINTABLE_RADIUS
...
// 수동으로 홈 위치를 설정합니다.자동 설정을 위해 정의되지 않은 상태로 두십시오.
// 델타의 경우 이것은 직교 인쇄 볼륨의 최고 중심입니다.
#define MANUAL_X_HOME_POS R_LINE_OFFSET
...
#define HOMING_FEEDRATE_MM_M { (500), (100), (4*60) }
...
#define EEPROM_BOOT_SILENT    // Keep M503 quiet and only give errors during first load
  ...
  //#define EEPROM_AUTO_INIT  // 모든 오류에 대해 EEPROM을 자동으로 시작하십시오.
  #define EEPROM_INIT_NOW   // 새 빌드 후 첫 부팅에서 EEPROM을 시작하십시오.
#endif
...
// ENCODER SETTINGS
//
//이 옵션은 필요한 인코더 펄스의 기본 수를 대체합니다.
// 한 단계를 생성합니다.고해상도 인코더의 경우 증가해야합니다.
//
#define ENCODER_PULSES_PER_STEP 4

//
//이 옵션을 사용하여 필요한 단계 신호 수를 재정의합니다.
// 다음/이전 메뉴 항목 사이를 이동합니다.
//
...
// 이것을 1으로 증가하면 소프트웨어 PWM 주파수가 두 배가됩니다.
// 히터에 영향을 미치고 fan_soft_pwm이 활성화 된 경우 팬이 있습니다.
// 그러나 각 증분에 대해 제어 해상도가 절반으로 줄어 듭니다.
// 0 값으로 128 개의 효과적인 제어 위치가 있습니다.
// :[0,1,2,3,4,5,6,7]
#define SOFT_PWM_SCALE 0

// soft_pwm_scale이 0보다 높은 값으로 설정된 경우 DiThering CAN
// 관련 해상도 손실을 완화하는 데 사용됩니다.활성화 된 경우
// 일부 PWM주기 중 일부는 평균적으로 원하는 것입니다.
// 듀티 사이클이 달성됩니다.
...
//온도 및 침대 온도를 나타내는 온도 상태 LED.
// 모든 뜨거운, 침대 온도 및 대상 온도가 54C 미만인 경우
// 그러면 파란색 LED가 켜져 있습니다.그렇지 않으면 빨간색 LED가 켜져 있습니다.(1C 히스테리시스)
